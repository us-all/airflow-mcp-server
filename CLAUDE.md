...
`@us-all/airflow-mcp` — Airflow Stable REST API를 stdio MCP로 노출. **16 도구 + 2 Prompts**. read-only 기본, trigger/clear는 `AIRFLOW_ALLOW_WRITE=true` 게이트.
...
│   ├── admin.ts            # 6 도구 (import-errors, dag-warnings, pools, variables, connections, event-logs) — 운영 진단 read
...
| 카테고리 | 도구 수 | 토글 키 |
|---------|--------|---------|
| `airflow` | 15 + 1 aggregation | `AIRFLOW_TOOLS=airflow` |
| `meta`    | 1 (always) | — |
